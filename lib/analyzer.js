...
function analyseSeasonData(gamesData, season) {
  ...
  gamesData.forEach(gameData => {
    ...
    totalOffensiveWins += analyseGame(faceoffs, homeTeamTriCode, isHomeTeamStartsOnLeft, season);
    totalFaceoffs += faceoffs.length;
  });

  console.log([
    `Offensive wins: ${totalOffensiveWins}`,
    `Total faceoffs: ${totalFaceoffs}`
  ].join('\n'));

  return totalOffensiveWins / totalFaceoffs;
}

function analyseGame(faceoffs, homeTeamTriCode, isHomeTeamStartsOnLeft, season) {
  let offensiveWins = 0;
  faceoffs.forEach(faceoff => offensiveWins += isOffensiveWon(faceoff, homeTeamTriCode, isHomeTeamStartsOnLeft, season));
...
function isOffensiveWon(faceoff, homeTeamTriCode, isHomeTeamStartsOnLeft, season) {
  // Center ice faceoff
  if (faceoff.coordinates.x === 0 || season < 20152016) {
